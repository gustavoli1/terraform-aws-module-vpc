...
#### Modifying Attachment Subnets

When you add or remove Availability Zones from your VPC configuration, the list of subnets associated with the Transit Gateway attachment is updated automatically. This is an in-place update, meaning the attachment's ID **does not change**. The underlying network interface in the affected AZ is simply created or destroyed as needed. You do not need to update any static routes in your Transit Gateway route table that point to this attachment.

The module includes a `create_before_destroy` lifecycle policy on the attachment resource. This policy ensures that if you make a change that *does* require replacing the attachment (such as changing the `transit_gateway_id` itself), the new attachment will be created before the old one is destroyed to minimize traffic disruption.
